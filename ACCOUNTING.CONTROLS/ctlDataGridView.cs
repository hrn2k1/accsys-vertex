    ...
    public partial class ctlDaraGridView : DataGridView 
    {
       ...
       Window = System.Security.Permissions.UIPermissionWindow.AllWindows)]
        protected override bool ProcessDialogKey(Keys keyData)
        {
            ...
            if (key == Keys.Enter && _isEnterKeyLikeTabKey==true)
            {
        ...
        {
            // Handle the ENTER key as if it were a RIGHT ARROW key. 
            if (e.KeyCode == Keys.Enter && _isEnterKeyLikeTabKey == true)
            {
        ...
        private bool _isEnterKeyLikeTabKey = true;
        private bool _isExcelSheetCell = false;
        #endregion

        #region Properties
        public bool isExcelSheetCell
        {
            get { return _isExcelSheetCell; }
            set { _isExcelSheetCell = value; }
        }
        public bool isEnterKeyLikeTabKey
        {
            get { return _isEnterKeyLikeTabKey; }
            set { _isEnterKeyLikeTabKey = value; }
        }
        ...
        private void this_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            try
            {
                if (e.Exception.GetType() == typeof(FormatException))
                {
                    string exp = GlobalFunctions.isNull(this[e.ColumnIndex, e.RowIndex].EditedFormattedValue, "");
                    e.Cancel = false;
                    if (_isExcelSheetCell == true)
                    {
                        //this.EndEdit();
                        ...
                        if (exp[0] == '=')
                        {
                            double d = getResult(exp.Substring(1));
                            this[e.ColumnIndex, e.RowIndex].Value = d;
                        }
                    }
