   ...
   public class DaAccount
    {
       ...
       public int InsertUpdateAccount(Account objAcc, SqlConnection con)
       {
           int ID = 0;
           try
           {
       ...
       public int InsertUpdateAccount(Account objAcc, SqlConnection con,SqlTransaction trans)
       {
           int ID = 0;
           try
           {
       ...
       public void UpdateLedgerID(SqlConnection con, SqlTransaction trans, int AccID, int LdgrID)
       {
           try
           {
               SqlCommand cmd = new SqlCommand("UPDATE T_Account SET LedgerID=@ldgrID WHERE AccountID=@accID", con, trans);
               if (LdgrID <= 0)
       ...
       public DataTable GetAccounts(SqlConnection con)
       {
           ...
           try
           {
               SqlDataAdapter da = new SqlDataAdapter("select * from T_Account WHERE CompanyID=@CompanyID AND accOrGroup like 'Account' ORDER BY AccountNo", con);
               da.SelectCommand.Parameters.Add("@CompanyID", SqlDbType.Int).Value = LogInInfo.CompanyID;
               da.Fill(ds);
               da.Dispose();
           }
       ...
       public DataTable getAccountHeads(SqlConnection con)
       {
           try
           {
               ...
               SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM T_Account WHERE CompanyID=@CompanyID AND AccOrGroup like 'Group' ORDER BY AccountTitle", con);
               da.SelectCommand.Parameters.Add("@CompanyID", SqlDbType.Int).Value = LogInInfo.CompanyID;
               da.Fill(dtAccHeads);
               da.Dispose();
               return dtAccHeads;
       ...
       public Account getSalesAccount(SqlConnection con, int AccountID)
       {
           Account obSalesAccount = new Account();
           DataTable dt = new DataTable();
           try
           {
       ...
       public Account getPurchaseAccount(SqlConnection con, int AccountID)
       {
           Account obPurchaseAccount = new Account();
           DataTable dt = new DataTable();
           try
           {
               SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM T_Account WHERE AccountID = @AccountID", con);
