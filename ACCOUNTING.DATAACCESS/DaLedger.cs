...
namespace Accounting.DataAccess
{
    public class DaLedger
    {
        ...
        public static DataTable GetLedgers(int companyId, int type)
        {
            var dt = new DataTable();
            try
            {
                string qstr = "SELECT LedgerID, LedgerName, T_Ledgers.LedgerTypeID, T_LedgerType.LedgerType FROM T_Ledgers INNER JOIN T_LedgerType ON T_Ledgers.LedgerTypeID = T_LedgerType.LedgerTypeID";
                qstr += string.Format(" WHERE T_Ledgers.CompanyID={0} ", companyId);
                if(type > 0)
                    qstr += string.Format(" AND T_Ledgers.LedgerTypeID ={0} ", type);
                qstr += " ORDER BY LedgerName";
                using (SqlDataAdapter da = new SqlDataAdapter(qstr, ConnectionHelper.getConnection()))
                {
                    da.Fill(dt);
                    da.Dispose();
                }
        ...
        public int InsertUpdateLedger(Ledgers objLdgr, SqlConnection con, SqlTransaction trans)
        {
            int ID = 0;
            try
            {
        ...
        public void UpdateAccountID(SqlConnection con, SqlTransaction trans, int LdgrID, int AccID)
        {
            try
            {
                SqlCommand cmd = new SqlCommand("UPDATE T_Ledgers SET AccountID=@accID WHERE LedgerID=@ldgrID", con, trans);
                if (AccID <= 0)
