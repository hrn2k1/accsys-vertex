...
namespace AccSys.Web
{
    public partial class frmSaleOrder : System.Web.UI.Page
    {
        ...
        private SaleOrderModel CreateModel()
        {
            var model = new SaleOrderModel
            {
                ...
                DeliveryDate = txtDeliveryDate.Text.Trim() == "" ? new DateTime(1900,1,1): Tools.Utility.GetDateValue(txtDeliveryDate.Text.Trim(), DateNumericFormat.YYYYMMDD),
                OrderNo = txtOrderNo.Text,
                LedgerNo = txtLedgerNo.Text,
                CurrencyId = ddlCurrency.SelectedValue.ToInt(),
                CurrencyRate = txtRate.Text.ToDouble(),
                OrderQty = txtTotalQty.Text.ToDouble(),
                OrderValue = txtTotalAmount.Text.ToDouble(),
                BuyerRef = txtBuyerref.Text,
                CustomerId = ddlCustomer.SelectedValue.ToInt(),
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                odsCommon.SelectParameters["SelectedColumns"].DefaultValue = @" OrderMID AS OrderID, OrderNo, OrderDate, CustomerID, LedgerName AS CustomerName, TotalOrderQty AS OrderQty, OrderValue, Buyer_ref, Order_Master.CompanyID ";
        ...
        protected void lbtnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtLedgerNo.Text = order.LedgerNo;
                txtDate.Text = order.OrderDate.ToString(_dateFormat);
                txtDeliveryDate.Text = order.DeliveryDate == new DateTime(1900,1,1) ? "" : order.DeliveryDate.ToString(_dateFormat);
