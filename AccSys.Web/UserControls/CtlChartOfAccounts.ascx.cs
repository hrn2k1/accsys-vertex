                ...
                foreach (DataRow row in dsGLs.Tables["Dir"].Rows)
                {
                    if (row.Field<object>("ParentID") == DBNull.Value || row.Field<object>("ParentID") == null)
                    {
                        if (row.Field<string>("AccOrGroup") == "Account")
                            node = new TreeNode(string.Format("<B style='color:red'>{0}</B>", row.Field<string>("AccountTitle")), row.Field<object>("AccountID").ToString(), "/images/small.png");
                        else
        ...
        private void AddNode(DataRow dr, TreeNode n)
        {
            try
            {
                ...
                foreach (DataRow r in dr.GetChildRows("DirRelation"))
                {
                    ...
                    if (r.Field<string>("AccOrGroup") == "Account") nn.ImageUrl = "/images/small.png";
                    n.ChildNodes.Add(nn);
                    AddNode(r, nn);
                }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (msgFlag == true)
                {
                    ...
                    if (ddlACType.SelectedValue == "Group")
                    {
                        ddlParent.Bind();
                        ddlSrcParent.Bind();
                    }
        ...
        protected void lbtnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (objA != null)
                {
                    ...
                    trans = connection.BeginTransaction();
                    if (objA.LedgerID > 0)
                        new DaLedger().DeleteLedger(objA.LedgerID, connection, trans);
                    new DaAccount().DeleteAccount(objA.AccountID, connection, trans);
                    trans.Commit();
                    connection.Close();
                    btnSearch_Click(sender, e);
                    if (objA.AccountOrGroup == "Group")
                        ddlParent.Bind();
                    lblPanel1Msg.Text = UIMessage.Message2User("Deleted successfully", UserUILookType.Success);
                }
            }
            catch (Exception ex)
            {
